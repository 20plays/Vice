...
def _stop_daemon_completely(timeout: float = 10.0) -> None:
    """Stop recorder ownership and wait without force-killing slow shutdowns."""
    ...
    if not _wait_for_daemon_exit(timeout=timeout):
        # Current upstream deliberately leaves a slow finalization alone.
        # Propagate failure so tray Quit keeps the GUI open for a retry.
